...
class Graph:
    ...
    def removeedge(self,u,v):
        ...
        self.rem.append((u,v))
        self.normal_bfs(u,self.max_grp+1)
        self.normal_bfs(v,self.max_grp+2)
        self.max_grp+=2

    def distancefinder(self,u,v):
        if(self.grps[u] != self.grps[v]):return "Impossible"
    ...
    def addoldedge(self,u,v):
        if((u,v) in self.rem ):
            self.graph[u] = self.graph.get(u,[]) +  [v]
            self.graph[v] = self.graph.get(v,[]) +  [u]
            del self.rem[ self.rem.index((u,v)) ]
            self.normal_bfs(u,self.max_grp+1)
            self.max_grp+=1
        elif((v,u) in self.rem):
            self.graph[u] = self.graph.get(u,[]) +  [v]
            self.graph[v] = self.graph.get(v,[]) +  [u]
            del self.rem[ self.rem.index((v,u)) ]
            self.normal_bfs(u,self.max_grp+1)
            self.max_grp+=1
